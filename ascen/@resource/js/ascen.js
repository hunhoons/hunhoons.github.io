...
function isInit(){
	...
	/* <Scroll Top> - 스크롤 탑 버튼 */
	$(document).on("click", "#scrollTopBtn", function(e){
		if(window.lenis){
			// 1. Lenis 일시 정지
			window.lenis.stop();
...
function forceScrollTop() {
	...
	document.body.scrollTop = 0; // iOS Safari 필수
}
function lockBody() {
	...
	window.addEventListener('load', () => {
		...
		requestAnimationFrame(() => {
			...
			requestAnimationFrame(() => {
				...
				// ✅ [핵심] stopAfter 옵션으로 load 후 stop 여부 결정
				if (stopAfter) {
					window.lenis.stop();
				}
			});
...
function isAccordion(event){ 				// 아코디언
	...
	accBtn.on('click', function(e){
		...
		if(event == 1){ // 1개씩 노출 될 때
			accBtn.parent('.accordion-area').removeClass('active');
...
/* <Footer Effect GSAP> - 푸터 등장 모션 */
function footerGsap(){
	setTimeout( () => {
		if(!document.querySelector('#footer')) return;

		gsap.set('#footer .foot-inner', { yPercent: -100, });
		gsap.to('#footer .foot-inner', {
			scrollTrigger:{
				trigger: '#footer',
				start: "top bottom",
				end: 'bottom bottom+=1',
...
function fadeUpGsap() {
	const fadeUp = gsap.utils.toArray('[data-gsap="fade-up"]');
	fadeUp.forEach((item) => {
		// 초기 상태 세팅
...
function fadeUpRandomGsap() {
	const fadeUp = gsap.utils.toArray('[data-gsap="fade-up"]');
	fadeUp.forEach((item) => {
		// 초기 상태 세팅
		gsap.set(item, { y: 30, autoAlpha: 0 });

		const randomStart = gsap.utils.random(80, 90, 1); // 숫자만 랜덤 생성
...
function fadeUpToggleGsap() {
	const fadeUp = gsap.utils.toArray('[data-gsap="fade-up-toggle"]');
	fadeUp.forEach((item) => {
...
function fadeUpTabsToggleGsap() {
	const fadeUp = gsap.utils.toArray('[data-gsap="fade-up-tab-toggle"]');
	fadeUp.forEach((item) => {
...
function fadeUpCenterGsap(pos) {
	const fadeUp = gsap.utils.toArray('[data-gsap="fade-up-center"]');
	fadeUp.forEach((item) => {
		// 초기 상태 세팅
		gsap.set(item, { y: 30, autoAlpha: 0 });
...
function fadeUpGsapMo() {
	const fadeUp = gsap.utils.toArray('[data-gsap="fade-up-mo"]');
	fadeUp.forEach((item) => {
		// 초기 상태 세팅
		gsap.set(item, { y: 30, autoAlpha: 0 });
...
function scaleGsap() {
	const imgScale = gsap.utils.toArray('[data-gsap="img-scale"]');
...
function motionMarqueeDuo({ target, texts = [], cnt, speed = 6 }) {
	...
	/* 
		motionMarqueeDuo({
			target	: '.orgs-about .marquee .inner',
