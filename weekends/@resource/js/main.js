...
/* {isLayoutReady} - header/footer include가 끝난 뒤에만 실행 */
...
$(function(){
	...
	/* {Lenis Lib} */
	window.lenis?.stop?.(); // 최초 측정이 끝날 때까지 스크롤을 잠시 정지.
		...
		(document.fonts?.ready || Promise.resolve()).then(() => {
			...
			requestAnimationFrame(() => {
				...
				measurements.forEach(({ textPath, phrase, phraseWidth, pathLength, wrap }) => {
					// 이전: Math.ceil(pathLength / phraseWidth) + 2  (시간이 지나면 텍스트가 잘려 보이던 버전)
					// startOffset이 0~100% 순환하는 동안 어느 지점에서도 path 끝까지 텍스트가 남아있으려면
					// 텍스트 총 길이가 path 길이의 2배 이상이어야 해서 *2 적용.
					const repeatCount = (!phraseWidth || !isFinite(phraseWidth) || !pathLength)
						? 30
						: Math.ceil((pathLength * 2) / phraseWidth) + 2;
					textPath.textContent = phrase.repeat(repeatCount);

					// 이전: gsap.ticker에서 매 프레임(최대 60fps) setAttribute('startOffset', ...) 호출
					// → path 위 전체 글자 재배치 연산이라 무거움. 30fps로 스로틀 + 경과시간(dt) 기반 이동으로 변경.
					const state = { offset: 0, last: 0, wasActive: false };
					const FRAME_INTERVAL = 1000 / 30;
					const SPEED_PER_SEC = 1.2; // 기존 "0.02 / tick(60fps)"와 체감 속도 동일
					const ticker = () => {
						const st = wrap?._collabTimeline?.scrollTrigger;
						if (!st || !st.isActive) { state.wasActive = false; return; }
						const now = performance.now();
						// 비활성 → 활성으로 막 전환된 첫 프레임: dt를 재지 않고 시각만 맞춰둠
						// (여기서 바로 dt를 재면 "페이지 로드 후 지난 전체 시간"이 dt로 잡혀 offset이 확 튐)
						if (!state.wasActive) { state.wasActive = true; state.last = now; return; }
						if (now - state.last < FRAME_INTERVAL) return;
						const dt = (now - state.last) / 1000;
						state.last = now;
						state.offset = (state.offset + SPEED_PER_SEC * dt) % 100;
